...
impl<B: GraphicsBackend> FemtoVGRenderer<B> {
    ...
    fn internal_render_with_post_callback(
        ...
        // 本项目 patch：Slint 以物理(设备)像素绘制（几何 = logical * scale_factor、
        // 字号为 PhysicalLength、画布 transform 无缩放），所以 femtovg 的 device
        // pixel ratio 应为 1.0——画布单位即设备像素，AA 羽化(fringe=1.0/dpi)正好
        // 1 设备像素。原实现传 ceil(scale_factor)，在 125% 等非整数缩放下 dpi=2.0、
        // fringe=0.5px，小图形（单选钮/复选框）边界锯齿明显。文本字号是物理的，
        // dpi=1.0 时字形按原生密度栅格化，尺寸不变且为整数 dpi（满足 femtovg 文本
        // 渲染对整数 device pixel ratio 的要求）。
        let scale = 1.0;
